...
const DeleteAccountModal = () => {
  ...
  const deleteAccount = async () => {
    ...
    const comments = await getDocs(
      query(collection(db, "comments"), where("userId", "==", session.user.id))
    );
    ...
    const posts = await getDocs(
      query(collection(db, "posts"), where("userId", "==", session.user.id))
    );
    posts.docs.length > 0 &&
      posts.docs.forEach(async (post) => {
        await deleteObject(
          ref(storage, `posts/images/${session.user.id}/${post.id}`)
        );
        await deleteDoc(doc(db, "posts", post.id));
      });

    // remove likes by the current users on posts
    const likedPosts = await getDocs(
      query(
        collection(db, "posts"),
        where("likes", "array-contains", session.user.id)
      )
    );
    likedPosts.docs.length > 0 &&
      likedPosts.docs.forEach(async (likedPost) => {
    ...
    currentUser.data()?.following.length > 0 &&
      currentUser.data()?.following.forEach(async (userId: string) => {
        await updateDoc(doc(db, "users", userId), {
          followers: arrayRemove(session.user.id),
        });
      });
    currentUser.data()?.followers.length > 0 &&
      currentUser.data()?.followers.forEach(async (userId: string) => {
        await updateDoc(doc(db, "users", userId), {
    ...
    const accounts = await getDocs(
      query(collection(db, "accounts"), where("userId", "==", session.user.id))
    );
    ...
    const sessions = await getDocs(
      query(collection(db, "sessions"), where("userId", "==", session.user.id))
    );
  ...
  return (
    <Modal open={openModal} onClose={() => setOpenModal(false)}>
      <>
        {deleting && (
          <div className="fixed inset-0 flex items-center bg-gray-900/30 justify-center">
            <ContentLoader />
          </div>
        )}
        <div className="flex flex-col">
          ...
          <button
            ...
            className="font-[600] text-red-500 py-3 border-b"
          >
            {deleting ? "Deleting..." : "Delete"}
          </button>
          <button onClick={() => setOpenModal(false)} className="py-3">
            Cancel
          </button>
        </div>
      </>
