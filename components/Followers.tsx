...
const Followers = ({ onClose, userId, onModal = false }: Props) => {
  ...
  return (
    <div
      className={`mb-6 ${!onModal && "mt-6 rounded-lg shadow-mainShadow bg-white pb-5"} md:mx-auto md:w-[500px] text-center`}
    >
      <div
        className={`h-[50px] px-4 border-b bg-white sticky ${onModal ? "top-0" : "top-[53px] md:rounded-tr-lg md:rounded-tl-lg"}`}
      >
      ...
      {followers.length > 0 ? (
              ...
              <div className="flex items-center">
                <div className="h-14 w-14 mr-4">
                  <Link href={`/${follower.data().username}`}>
                    <img
                      src={follower.data().image}
                      alt=""
                      className="h-full object-cover rounded-full border
                ...
                <div className="flex flex-col items-start">
                  <div className="flex items-center">
                    <Link
                      href={`/${follower.data().username}`}
                      className="font-bold"
                    >
                      {follower.data().username}
                    </Link>
                  </div>
                  <p className="font-[600] text-gray-400">
                    {follower.data().name}
                  </p>
                </div>
              </div>
        ...
        <p className="text-gray-400 mt-5">
          This user does not have a follower right now
        </p>
      )}
    </div>
  );
