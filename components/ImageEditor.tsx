...
const ImageEditor = ({
  ...
  return (
    <div className="md:max-w-[800px]">
      {alertChanges && <PostDiscardAlert discard={discard} cancel={cancel} />}
      <header className="p-5 border-b border-solid border-b-gray-200 flex justify-between items-center">
        ...
        <button
          className="text-instaBlue font-bold pr-2"
          onClick={() => onNext()}
        >
          Next
        </button>
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div
          className="md:basis-1/3 my-5 md:my-0 md:h-auto mx-2 md:mx-0 md:mt-0 flex md:grid md:grid-cols-2 
                    md:max-h-[550px] md:overflow-y-auto overflow-x-auto scrollbar-none"
        >
          <figure
            onClick={() => setFilter("")}
            className="min-w-[75px] sm:min-w-[90px] md:min-w-[auto]"
          >
            <img
              src={selectedFile}
          ...
          {imageFilters.map((filter, idx) => (
            <figure
              ...
              className={
                filter + " min-w-[75px] sm:min-w-[90px] md:min-w-[auto]"
              }
            >
              <img
                src={selectedFile}
                className="md:h-full object-contain relative"
                alt={`filter ${filter}`}
              />
            </figure>
          ))}
