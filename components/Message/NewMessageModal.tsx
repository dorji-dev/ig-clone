...
const NewMessageModal = () => {
  ...
  const [chatUsers, setChatUsers] = useState<
    { name: string; username: string; image: string }[] | null
  >(null);
          ...
          <div className="grow flex flex-wrap">
            {chatUsers &&
              chatUsers.map((chatUser) => (
                <div
                  key={uuidv4()}
                  className="flex items-center rounded-full bg-instaBlue/10 px-3 py-1 mr-2 mb-2"
                >
                  <span className="text-instaBlue mr-2">
                    {chatUser.username}
                  </span>
                  <button
                    onClick={() => {
