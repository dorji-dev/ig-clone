...
const Notifications = ({
  ...
  const [notis, setNotis] = useState<
    { image: string; username: string; message: string; follows: boolean }[]
  >([]);
    ...
    <div className={`md:max-w-[500px]`}>
      ...
      >
        <div className="h-full float-left flex items-center relative z-10">
          <button className={`${onModal && "hidden"}`} onClick={onClose}>
      ...
      {/* main content */}
      <section
        className={`py-5 bg-white ${!onModal && "shadow-mainShadow rounded-bl-lg rounded-br-lg"}`}
      >
        <p className="text-gray-400 text-sm text-center px-5 py-3">
          These are fake notifications for now. Yet to implement!
        </p>
        {notis ? (
          notis.map((noti) => (
            <div
            ...
            >
              ...
              <div className="w-fit ml-auto">
                {noti.follows ? (
                  <span className="px-4 py-2 border rounded-md font-[600]">
                    Following
                  </span>
                ) : (
                  <button
