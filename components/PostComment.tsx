          ...
          <div className="flex mb-7">
            <div className="mr-5">
              <Link href={`/${comment.username}`} className="rounded-full">
                <img
                  src={comment.userImage ?? "/images/placeholder.png"}
                  alt="commenter image"
            ...
            <div className="flex-1">
              <p>
                <Link href={`/${comment.username}`} className="font-bold mr-3">
                  {comment.username}
                </Link>
                <span>{comment.text}</span>
              </p>
              <div className="flex text-gray-400 text-sm mt-2">
                <Moment fromNow className="mr-4">
                  {comment.timeStamp?.toDate()}
                </Moment>
                <span className="mr-4 font-bold">
                  {comment.likes.length} likes
                </span>
                <button onClick={() => reply("ref")} className="font-bold">
                  Reply
                </button>
              </div>
            </div>
